...
- `script.js`：开屏验证、提前照片预加载、滚动进度、入场动画和按钮反馈。
- `photos/`：桌面端和兜底使用的照片。
- `photos/mobile/`：手机端优先加载的轻量照片。

## 开屏验证和加载

页面打开后会先显示验证层，同时立即开始加载照片。输入 `赵霞` 只负责解锁内容；如果照片还没有加载完，会继续停留在开屏页等进度到 100%。
...
脚本初始化时会读取页面里的所有 `<img>`，逐张预加载，并更新开屏进度条。手机屏幕会优先预加载 `data-mobile-src` 指向的小图。名字正确且照片加载完成后，按钮会变成“进入拾光集”，这时才会解锁正文。
